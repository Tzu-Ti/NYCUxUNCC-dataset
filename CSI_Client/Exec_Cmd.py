...
class Exec_Cmd():
    ...
    def exec(self, cmd: str):
        cmd = cmd.split(' ')
        if cmd[0] == 'stdby':
            #self.run(f'array_prepare_for_picoscenes 3 {cmd[2].replace("_", " ")}')
            print(f'echo \'array_prepare_for_picoscenes 3 {cmd[2].replace("_", " ")}\'')
            ...
            if cmd[1] == 'RX':
                self.process = subprocess.Popen(f'cd {cmd[2]} && PicoScenes "-i 3 --mode logger"', shell=True, preexec_fn=os.setsid)
            elif cmd[1] == 'TX':
                self.process = subprocess.Popen(f'cd {cmd[2]} && PicoScenes "-i 3 --mode injector --preset TX_CBW_160_HESU --repeat 1e5 --delay 5e3"', shell=True, preexec_fn=os.setsid)
